...
class ConfluenceScalpingStrategy extends BaseStrategy {
  constructor(positionSize = 0.1) {
    ...
    this.minConfluenceScore = 3; // Keep same confluence requirement
  ...
  generateSignal(currentCandle, historicalData, portfolio) {
    ...
    for (const signalType of ['LONG', 'SHORT']) {
      ...
      if (this.debugCount < 3) {
        console.log(`Confluence ${signalType}: Score ${confluenceResult.totalScore}/6`, confluenceResult.scores);
        this.debugCount++;
      }
          ...
          if (signalType === 'LONG') {
            ...
            if (currentPosition.quantity <= 0) {
              ...
              return {
                ...
                reason: `Confluence LONG ${confluenceResult.totalScore}/6: ${Object.entries(confluenceResult.scores).filter(([k,v]) => v > 0).map(([k,v]) => k).join(', ')}`,
            ...
            if (currentPosition.quantity >= 0) {
              ...
              return {
                action: 'SELL',
                quantity: baseQuantity,
                reason: `Confluence SHORT ${confluenceResult.totalScore}/6: ${Object.entries(confluenceResult.scores).filter(([k,v]) => v > 0).map(([k,v]) => k).join(', ')}`,
