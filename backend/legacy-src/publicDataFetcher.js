...
class PublicDataFetcher extends EventEmitter {
  ...
  handleWebSocketMessage(message) {
    ...
    if (message.type === 'subscriptions') {
      console.log('✅ PUBLIC WebSocket subscription confirmed:', JSON.stringify(message.channels));
      return;
    }
    ...
    // Handle regular ticker updates (secondary channel)
      ...
      currentPrice = parseFloat(message.close || message.mark_price || message.last_traded_price);
      currentVolume = parseFloat(message.volume_24h || message.volume || 0);
      priceSource = 'ticker';
    ...
    // Log other message types for debugging (limit to avoid spam)
      ...
      console.log(`📨 PUBLIC WebSocket message (${message.type}):`, JSON.stringify(message).substring(0, 200));
    }
  }
